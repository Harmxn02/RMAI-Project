...
\begin{figure}[ht]
	...
	\fbox{\includegraphics[width=0.80\linewidth]{images/Diagram_V2.drawio.pdf}}
...
\label{subsec:models-configurations}
...
\begin{table}[H]
	...
	\end{tabular}
	\vspace{0.5em}
	\caption{Overview of all evaluated models}
	\label{tab:model-summary}
\end{table}

% Model summary table: see tables.tex, Table~\ref{tab:model-summary} (tab:model-summary). Source: config/model_info.yaml in the LLM Validator submodule. Note smollm2:135m was also run (full data in results/experiments.csv) but has no model_info.yaml entry, so it's excluded from this table and from every reasoning/parameter-count-based analysis in the Results section -- worth a one-line mention if you want to account for all 7 models tested rather than just the 6 with confirmed reasoning-mode metadata.
...
The validator checks structural and syntactic conformance to the HTML specification. A limitation of the validator is that it cannot evaluate semantic intent. For example, it cannot tell you that \verb|<div class="nav">| is worse than \verb|<nav>| because both are syntactically valid.
...
Since all models run locally via Ollama, there is no API pricing to measure. Instead, we proxy cost through three values captured per iteration: \verb|gen_time_s| (wall-clock generation time), \verb|prompt_eval_count| (tokens in the prompt), and \verb|eval_count| (tokens generated). These together give a picture of both time cost and computational load per run.
